...
| 11 | Major | /puzzle-author, /puzzle-test | These skills don't leverage /puzzle as data layer. | AoE Stage 4 |
| 12 | Major | Scenario structure | Need world/ directory with verified reference data. | AoE Stage 5 |
| 13 | Major | Pipeline (hunt/plan) | Missing EDITORIAL REVIEW stage between Authoring and Testing. In multi-author hunts, admin must review submissions before blind testing: check brief compliance, flag errors (Speedster), trim verbosity (Methodical), evaluate deviations (Skeptic). Without this, bad puzzles burn test cycles. Pipeline should be 11 stages. | Boardgames Stage 7 |
