...
| 18 | Major | Stage 4 ASSIGNMENT / brief issuing | Answer words must be validated against world data before briefs are issued. During IRONFALL Stage 6, P01 was given answer USHER and P02 given answer QUELL — but USHER couldn't be extracted from any enemy name combination, and QUELL contained letters (Q, U) that appear nowhere in the crafting item database. Both required post-authoring redesign. Fix: at brief-issue time, the author (or admin) must verify that the target answer can be physically extracted from the puzzle's content domain. Add a "mechanically feasible?" check to the brief template. | IRONFALL Stage 6 |
